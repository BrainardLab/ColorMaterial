% function params = ColorMatExp2Driver(exp)
function params = ColorMatExp2Driver(exp)
...
% 08/08/16   ar Adapting part1 to part 2
...
try
    ...
    %list = load([exp.subject 'stimulusList2b.mat']); 
    % first part of the experiment. 
    list = load([exp.subject 'stimulusList2b.mat']); % second part of the exp. 
    params.nTrials = length(list.imageList);
    
    % Target is fixed across trials and predefined in the config file.
    target = load([exp.subject params.targetName '-RGB.mat']);
    ...
    for i = 1: length(params.trialIndices);
        ...
        stimulusOneName{1} = [list.imageList{orderIndex}(6:14)];
        sOne = load([stimulusOneName{1} '-RGB.mat']);
        stimulusTwoName{1} = [list.imageList{orderIndex}(16:end)];
