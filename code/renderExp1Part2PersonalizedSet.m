...
for xx = 1:length(subjectList)

subjectName = subjectList{xx};

% Choose batch renderer options.
hints.imageWidth = 1280;
hints.imageHeight = 960;
hints.renderer = 'Mitsuba';
hints.recipeName = [subjectName 'Part2StimulusSetBlobbyExp1-' date];
...
for c = 1:nConditions
    ...
    for t = 1:nTests
        ...
        values{1} = [subjectName, condCode 'C' num2str(cCode), 'M' num2str(mCode)];
        values{3} = [subjectName, 'Part2' condCode 'CompetitorBlobRef' num2str(cCode) '.spd'];
        ...
        imageList = [imageList, values{1}];
    end
    % save stimulus list
    emailToStr = 'radonjic@sas.upenn.edu';
    setpref('Internet', 'SMTP_Server', 'smtp-relay.upenn.edu');
    setpref('Internet', 'E_Mail', emailToStr);
    sendmail(emailToStr, 'RenderDone', 'Finished one condition for this subject.');
end
cd (currentDir)
save([subjectName, 'MSList'], 'imageList');
end