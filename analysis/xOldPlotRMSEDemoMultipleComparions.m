...
for i = 1:length(simulateWeigth)
    for whichLoop = 1:nLoops
        F1 = load(['/Users/ana/Dropbox (Aguirre-Brainard Lab)/CNST_analysis/ColorMaterial/demoPlots/Apr19/' interCode num2str(nBlocks1) 'saveRetParamsFixedAddOn.mat']);
        F2 = load(['/Users/ana/Dropbox (Aguirre-Brainard Lab)/CNST_analysis/ColorMaterial/demoPlots/' interCode num2str(nBlocks1) 'saveRetParamsFixedAddOn2.mat']);
       
        %F1a =   load(['L' num2str(nBlocks1) 'returnedParamsFixed.mat']);
        V1(whichLoop) = load([interCode num2str(nBlocks1) 'saveRetParamsVaryAddOn' num2str(whichLoop) '.mat']);
        V2(whichLoop) = load([interCode num2str(nBlocks1) 'saveRetParamsVary' num2str(whichLoop) '.mat']);
...
for i = 1:3
    for whichLoop = 1:nLoops
        ...
        plot(V2(whichLoop).params.materialMatchColorCoords, V2(whichLoop).returnedParams{i}(8:14),'ko-', 'MarkerSize', thisMarkerSize);
        ...
        plot(V2(whichLoop).params.colorMatchMaterialCoords, V2(whichLoop).returnedParams{i}(1:7),'ko-', 'MarkerSize', thisMarkerSize);
...
comparison  = abs(recoveredWVary1 - repmat(simulateWeigth, [nLoops,1])); 
