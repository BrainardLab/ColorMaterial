...
%  1: Full model
%  2: Cubic model
tryWeightValues = round(linspace(0.01,0.99, 10),2); 
nModelTypes = length(tryWeightValues);
...
%         end
        ...
        switch subjectList{ss}
            case 'jcd'
                ...
                modelCode = ['Linear' 'W' num2str(params.tryWeightValues)];
            case 'dca'
                params.whichPositions = 'full';
                params.tryWeightValues = tryWeightValues(whichModelType);
                modelCode = ['Full' 'W' num2str(params.tryWeightValues)];
            case 'ofv'
                params.whichPositions = 'full';
                params.tryWeightValues = tryWeightValues(whichModelType);
                modelCode = ['Full' 'W' num2str(params.tryWeightValues)];
            ...
            case 'cjz'
                ...
                params.smoothOrder = 3;
                params.tryWeightValues = tryWeightValues(whichModelType);
                modelCode = ['Cubic' 'W' num2str(params.tryWeightValues)];
        ...
        cd(analysisDir);
        save([subjectList{ss} '-' num2str(nFolds) 'FoldsCV' '-'  modelCode  '-' params.whichDistance '.mat'], ...
